...
from structures import Avl, BinaryTree
...
def eq(a: _AnyNum, b:_AnyNum, accuracy: _AnyNum = 1e-6) -> bool:...
...
_ExpType = str | List[Complex, Literal["+", "-", "*", "/", "**", "log"]]
_GetExpTurnType = Complex | ValueError
_GetExpType = Callable[[_ExpType], _GetExpTurnType]
INFIX: str
SUFFIX: str
PREFIX: str
_ExpTypeType = Literal["infix", "suffix", "prefix"]

def split_exp(exp: str) -> _ExpType:...
def join_exp(exp: _ExpType) -> str:...

def getSuffix(exp: _ExpType) -> _GetExpTurnType:...
def getPrefix(exp: _ExpType) -> _GetExpTurnType:...
def getInfix(exp: _ExpType) -> _GetExpTurnType:...

def getExpressionType(exp: _ExpType) -> Tuple[_ExpTypeType, _GetExpType]:...

def toSuffix(exp: _ExpType) -> _ExpType:...
def toPrefix(exp: _ExpType) -> _ExpType:...
def toInfix(exp: _ExpType) -> _ExpType:...

class Expression(BinaryTree):
    value: int | None # If exp is Complex
    @overload
    def __init__(self, exp: _ExpType | Complex) -> None:...
    @overload
    def __init__(self, token: _ExpType, left: Expression | None, right: Expression | None) -> None:...
    @overload
    def __init__(self, token: _ExpType) -> None:...
    def getExpression(self, typ: _ExpTypeType) -> Complex:...
